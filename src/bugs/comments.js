...
import { checkCommentCreationAllowed } from "@/bugs/limits";
import {
...
export async function listComments(publicId) {
  ...
  return rows.map(mapComment).map((comment) => ({
    ...comment,
...
export async function createComment({ publicId, actorUserId, canWrite, bypassLimits = false, formData }) {
...
export async function updateComment({ commentId, actorUserId, canManage = false, content }) {
